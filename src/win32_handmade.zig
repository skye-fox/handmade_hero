...
fn win32ProcessXInputDigitalButton(old_state: *platform.GameButtonState, new_state: *platform.GameButtonState, xinput_button_state: win.DWORD, button_bit: win.DWORD) void {
    ...
    if (wam.RegisterClassA(&window_class) != 0) {
        ...
        if (window_handle) |window| {
            ...
            while (global_running) {
                var input = std.mem.zeroes([2]platform.GameInput);
                var new_input: *platform.GameInput = &input[0];
                var old_input: *platform.GameInput = &input[1];
                ...
                while (controller_index < max_controller_count) : (controller_index += 1) {
                    const old_controller: *platform.GameControllerInput = &old_input.controllers[controller_index];
                    const new_controller: *platform.GameControllerInput = &new_input.controllers[controller_index];
                ...
                const alpha: i32 = 0;
                platform.gameUpdateAndRender(new_input, &buffer, &game_sound_output_buffer, alpha);
                ...
                const temp: *platform.GameInput = new_input;
                new_input = old_input;
                old_input = temp;
            }
        } else {
